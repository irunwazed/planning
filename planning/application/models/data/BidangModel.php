...
class BidangModel extends CI_Model
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this->jumlah = 2000;
        $this->table = 'tb_bidang';
    }

    public function setQuery($post){
        $kode = explode("-", $post['kode']);
        $this->db->join('tb_fungsi', 'tb_fungsi.tb_fungsi_kode = '.$this->table.'.tb_fungsi_kode', 'left');
    ...
    public function create($post){
        ...
        if($this->cekInput($post)){
            ...
            $data = array(
                ...
                'tb_bidang_kode' => $post['tb_bidang_kode'],
                'tb_bidang_nama' => $post['tb_bidang_nama'],
                'tb_fungsi_kode' => $post['tb_fungsi_kode'],
            );
