...
class KegiatanController extends CI_Controller {
    ...
    {
		parent::__construct();
        $this->load->model('data/KegiatanModel');
    ...
    public function view($kode){
        ...
        $data['dataSotk'] = $this->DataModel->getRowProgram2($kode);
        $data['dataRpjmd'] = $this->DataModel->getRowVisi();
        $data['dataFungsi'] = $this->DataModel->getDataFungsi();

        $file['content'] = $this->load->view('components/data-kegiatan/content', $data, true);
        $file['script'] = $this->load->view('components/data-kegiatan/script', $data, true);
