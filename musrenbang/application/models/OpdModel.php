...
class OpdModel extends CI_Model
{
    ...
    public function setQuery($post){
        ...
        $Kd_Opd = explode("-", $post['session']['kd_opd']);
        $this->db->where($this->table.'.Kd_Urusan', $Kd_Opd[0]);
        $this->db->where($this->table.'.Kd_Bidang', $Kd_Opd[1]);
        $this->db->where($this->table.'.Kd_Unit', $Kd_Opd[2]);
        $this->db->where($this->table.'.Kd_Sub', $Kd_Opd[3]);

        $this->db->like($this->table.'.nama', @$post['search']);

        $this->db->join('ref_sub_unit', '
            ref_sub_unit.Kd_Urusan = '.$this->table.'.Kd_Urusan
            AND ref_sub_unit.Kd_Bidang = '.$this->table.'.Kd_Bidang
            AND ref_sub_unit.Kd_Unit = '.$this->table.'.Kd_Unit
            AND ref_sub_unit.Kd_Sub = '.$this->table.'.Kd_Sub
            ', 'left');
        ...
        if(@$post['kecamatan'] && count(explode("-", @$post['kecamatan'])) == 3){
        ...
        if(@$post['jenis'] != ""){
            if($post['jenis'] == 1){
                $this->db->where($this->table.'.asal <=', 2);
            }else if($post['jenis'] == 3){
                $this->db->where($this->table.'.asal', 3);
            }
        }
    ...
    public function getAll($post, $all = false, $print = false){
        ...
        if($print){
            $this->db->where($this->table.'.opdTerima >=', 1);
    ...
    public function usulanTerima($post){
        ...
        // print_r($post);
        $data = array(
            'opdTerima' => $post['opdTerima'],
            'opdTerimaKet' => $post['opdTerimaKet'],
        );
